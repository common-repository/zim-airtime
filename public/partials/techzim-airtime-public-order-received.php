...
<?php
$order_id = ( isset( $_REQUEST['techzim_airtime_order_id'] ) && ! empty( $_REQUEST['techzim_airtime_order_id'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['techzim_airtime_order_id'] ) ) : '';
...
$order_received_notice = get_option( $this->techzim_airtime . '-order-received-notice' );
$order_received_notice_display = '';
if ($order_received_notice !== '' || $order_received_notice !== null ) {
	$order_received_notice_display = '<p><em>'.$order_received_notice.'</em></p>';
} ?>
		<h3>Thank you!<br><br>Your Techzim Order ID is <?php echo esc_html( $order_id ); ?></h3>
		<?php echo $order_received_notice_display; ?>
		...
		if ($pinless_recharge == 'false') {
			...
			<?php } else { ?>
				<p> Due to NetOne challenges, we can't recharge your number directly. We are getting your recharge PINs ready. Please wait a second and then click 'Refresh' to load your PINs<br>
					<a href="<?php echo home_url().'/airtime-order-received/?techzim_airtime_order_id='.$order_id.'&pinless=false'; ?>">
